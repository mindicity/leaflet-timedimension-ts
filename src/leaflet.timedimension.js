import {Layer, Evented, Map, Util} from "leaflet";
import {
    explodeTimeRange,
...
export const TimeDimension = Layer.extend({
    ...
    _checkSyncedLayersReady: function (time) {
        for (var i = 0, len = this._syncedLayers.length; i < len; i++) {
            if (this._syncedLayers[i].isReady && !this._syncedLayers[i].isReady(time)) {
                return false;
            }
        }
        return true;
    ...
    _generateAvailableTimes: function () {
        if (this.options.times) {
            return parseTimesExpression(this.options.times);
        } else if (this.options.timeInterval) {
            var tiArray = parseTimeInterval(this.options.timeInterval);
            var period = this.options.period || "P1D";
            var validTimeRange = this.options.validTimeRange || undefined;
            return explodeTimeRange(tiArray[0], tiArray[1], period, validTimeRange);
        } else {
            return [];
        }
    },
    ...
    setAvailableTimes: function (times, mode) {
        ...
        if (mode === "extremes") {
            ...
            var parsedTimes = parseTimesExpression(times);
            if (this._availableTimes.length === 0) {
                this._availableTimes = parsedTimes;
            } else if (mode === "intersect") {
                this._availableTimes = intersect_arrays(
                    parsedTimes,
                    this._availableTimes
                );
            } else if (mode === "union") {
                this._availableTimes = union_arrays(parsedTimes, this._availableTimes);
