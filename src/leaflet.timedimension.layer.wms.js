import { Layer, Evented, Map, Util, TileLayer } from "leaflet";
import {
  explodeTimeRange,
...
export const TimeDimensionWMSLayer = TimeDimensionLayer.extend({
  initialize: function (layer, options) {
    ...
    this._wmsVersion =
      this.options.wmsVersion ||
      this.options.version ||
      layer.options.version ||
      "1.1.1";
    ...
    this._getCapabilitiesAlternateLayerName =
      this.options.getCapabilitiesLayerName || null;
    ...
    this._updateTimeDimensionMode =
      this.options.updateTimeDimensionMode || "intersect"; // 'union' or 'replace'
  ...
  getEvents: function () {
    var clearCache = Util.bind(this._unvalidateCache, this);
    return {
      moveend: clearCache,
      zoomend: clearCache,
    };
  ...
  isReady: function (time) {
    ...
    if (this.options.bounds && this._map)
      if (!this._map.getBounds().contains(this.options.bounds)) return true;
  ...
  setOpacity: function (opacity) {
    TimeDimensionLayer.prototype.setOpacity.apply(this, arguments);
  ...
  setZIndex: function (zIndex) {
    TimeDimensionLayer.prototype.setZIndex.apply(this, arguments);
    // apply to all preloaded caches
    for (var prop in this._layers) {
  ...
  setParams: function (params, noRedraw) {
    Util.extend(this._baseLayer.options, params);
  ...
  _requestTimeDimensionFromCapabilities: function () {
    ...
    oReq.addEventListener(
      ...
      function (xhr) {
        ...
        if (data !== null) {
          this._defaultTime = Date.parse(
            this._getDefaultTimeFromCapabilities(data)
          );
          ...
          this.setAvailableTimes(
            this._parseTimeDimensionFromCapabilities(data)
          );
  ...
  _getCapabilitiesUrl: function () {
    ...
    var params = Util.extend({}, this._getCapabilitiesParams, {
      request: "GetCapabilities",
      service: "WMS",
      version: this._wmsVersion,
    });
    url = url + Util.getParamString(params, url, params.uppercase);
    return url;
  },
  ...
  _getDefaultTimeFromCapabilities: function (xml) {
    ...
    if (layer) {
      ...
      if (defaultTime == 0) {
        defaultTime = this._getDefaultTimeFromLayerCapabilities(
          layer.parentNode
        );
      }
    }
  ...
  setAvailableTimes: function (times) {
    this._availableTimes = parseTimesExpression(times, this._period);
    this._updateTimeDimensionAvailableTimes();
  },

  _updateTimeDimensionAvailableTimes: function () {
    if (
      (this._timeDimension && this._updateTimeDimension) ||
      (this._timeDimension &&
        this._timeDimension.getAvailableTimes().length == 0)
    ) {
      this._timeDimension.setAvailableTimes(
        this._availableTimes,
        this._updateTimeDimensionMode
      );
...
TileLayer.include({
  ...
  _originalUpdate: TileLayer.prototype._update,
