...
export const TimeDimensionGeoJsonLayer = TimeDimensionLayer.extend({
  initialize: function (layer, options) {
    ...
    this._updateTimeDimensionMode =
      this.options.updateTimeDimensionMode || "extremes"; // 'union', 'replace' or extremes
    ...
    this._updateCurrentTime =
      this.options.updateCurrentTime || this._updateTimeDimension;
  ...
  _update: function () {
    ...
    if (this._duration) {
      ...
      subtractTimeDuration(date, this._duration, true);
      minTime = date.getTime();
    }

    // new coordinates:
    var layer = GeoJSON(null, this._baseLayer.options);
  ...
  _setAvailableTimes: function () {
    ...
    this._availableTimes = sort_and_deduplicate(times);
    this._updateCurrentTime =
      this._updateCurrentTime ||
      (this._timeDimension &&
